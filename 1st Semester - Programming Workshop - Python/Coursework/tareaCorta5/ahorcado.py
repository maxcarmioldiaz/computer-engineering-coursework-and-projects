...
def imprimirEntrada():
    ...
    print(" _______  __   __  _______  ______    _______  _______  ______   _______ ")
    print("|   _   ||  | |  ||       ||    _ |  |       ||   _   ||      | |       |")
    print("|  |_|  ||  |_|  ||   _   ||   | ||  |       ||  |_|  ||  _    ||   _   |")
    print("|       ||       ||  | |  ||   |_||_ |       ||       || | |   ||  | |  |")
    print("||       ||  |_|  ||    __  ||      _||       || |_|   ||  |_|  |")
    print("|   _   ||   _   ||       ||   |  | ||     |_ |   _   ||       ||       |")
    print("|__| |__||__| |__||_______||___|  |_||_______||__| |__||______| |_______|")
...
def leerIntento(intentadas):
    ...
    while len(letra) != 1 or letra not in "abcdefghijklmnñopqrstuvwxyz" or letra in intentadas:
        print(f"Por favor ingrese una letra que no haya intentado.{intentadas}")
        letra = input("Digite una letra: ")
        letra = letra.lower()
    print()
...
def imprimirMensajeVictoria(textoOriginal):
    ...
    Restricciones: ninguna"""
    print("Felicidades! Ha adivinado el texto:")
